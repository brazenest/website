...
export const RequestTeardownForm = component$(() => {
  ...
  const handleSubmit$ = $(async (event: SubmitEvent, element: HTMLFormElement) => {
    const form = element
  ...
  return (
    <form preventdefault:submit onSubmit$={handleSubmit$} class="flex flex-col gap-5 md:gap-6">
      ...
      {formState.value === 'success' && (
        <div class="rounded-[var(--radius-lg)] border border-green-200/30 bg-green-50/30 px-4 py-3 text-sm text-[var(--fg)]">
          Thanks for reaching out! I'll review your site and send a breakdown soon.
        </div>
      )}
      ...
      <button
        type="submit"
        disabled={formState.value === 'submitting' || formState.value === 'success'}
        class="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--fg)] px-4 py-2.5 text-sm font-medium text-[var(--bg)] transition-all duration-[var(--motion-duration-quick)] ease-[var(--motion-easing-quick)] hover:bg-[var(--state-hover-accent)] disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {formState.value === 'submitting' && 'Sending...'}
        {formState.value === 'success' && 'Sent!'}
        {(formState.value === 'idle' || formState.value === 'error') && 'Send Teardown Request'}
      </button>
    </form>
  )
})
